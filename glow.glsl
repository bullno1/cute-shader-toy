layout (set = 3, binding = 1) uniform shd_uniforms {
	// @param name=glow_size type=float2 min=0.0 default.x=0.6 default.y=0.6 step=0.1
	vec2 glow_size;
};
...
vec4 shader(vec4 color, ShaderParams params) {
	...
	vec4 glow_color = params.attributes;
	...
	for (float x = x_min; x <= x_max; x += 0.01) {
		for (float y = y_min; y <= y_max; y += 0.01) {
			vec2 sampled_uv = clamp(vec2(x, y) / u_texture_size, params.uv_min, params.uv_max);
			vec4 neighbor_color = texture(u_image, sampled_uv);
